...
ErrorCode_t Codec_Tasks(void)
{
    if (g_codec.tasks)
    {
        ...
        {
            CODEC_SetMute(codecHandle, kCODEC_PlayChannelHeadphoneLeft | kCODEC_PlayChannelHeadphoneRight, 1);
            g_codec.tasks &= ~CODEC_TASK_MUTE;
        }
        if (g_codec.tasks & CODEC_TASK_UNMUTE)
        {
            CODEC_SetMute(codecHandle, kCODEC_PlayChannelHeadphoneLeft | kCODEC_PlayChannelHeadphoneRight, 0);
